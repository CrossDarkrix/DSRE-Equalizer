...
**DSRE-Equalizer** は、Android上で再生中の音声を取得し、リアルタイムに軽量な音響補助処理を行う実験的なアプリです。

現在の安定版では、AudioPlaybackCaptureベースの Route A を使用し、元の再生音に対して **Wet Delta / Air Assist** 方式の補助音を薄く重ねる設計になっています。
...
そのため、v1.0.5ではフルレンジの加工音をそのまま重ねるのではなく、加工によって生じた差分成分や高域寄りの補助成分を小さく出力する方向へ調整しています。
...
v1.0.5では、テスト用のデバッグ欄や開発用ボタンを整理し、通常利用に必要な操作を中心にしたシンプルなUIへ変更しています。
...
v1.0.5では、この問題を軽減するため、フルレンジ出力ではなく Wet Delta / Air Assist 方式を採用しています。

---

## 実験済みの別ルートについて

開発中に、Android AudioEffect session を利用して他アプリの音声セッションに直接エフェクトを接続するD2ルートも検証しました。

しかし、検証環境では一部プレーヤーから外部AudioEffect sessionを取得できず、安定した汎用ルートとしては採用していません。

そのため、v1.0.5では AudioPlaybackCapture ベースの Route A を主軸とし、Wet Assist方式で実用性を高める方針にしています。
...
- Route A Wet Assistを安定版として整理
- Mode 7: Wet Delta を追加
- Mode 8: Air Assist を追加
- UIから主要パラメータを変更可能に変更
- デバッグ表示欄を削除
- 開発用ボタンを削除
- ファイルログ出力を抑制
